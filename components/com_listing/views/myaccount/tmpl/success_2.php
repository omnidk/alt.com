   ...
   $user_id=$user->get('id');
	...
	if ($final['ACK'] == 'Success')
	{
		...
		if($total_balance_and_line_credit > $amount)
		{
			...
			{
				if($amount > $userinfo['total_balance'])
				{
					$from_total_balance  = $userinfo['total_balance'];
					$from_line_of_credit = $amount - $userinfo['total_balance'];
				}
				else
				{
					$from_total_balance  = $amount;
					$from_line_of_credit = 0;
				}
				//insert transaction history
				$db->setQuery("INSERT INTO #__users_transfer_history SET id = '', amount='" . $amount . "', from_total_balance='" . $from_total_balance . "', from_line_of_credit='" . $from_line_of_credit . "', sender_id='" . $user_id . "', receiver_id='" . $receiver_id . "', comments='" . $comments . "', created_date='" . date('Y-m-d H:m:s') . "'");
				$db->query();
				...
				$db->setQuery("UPDATE #__barter_users_history SET total_balance = total_balance + total_balance + $amount,line_of_credit = line_of_credit WHERE user_id = $receiver_id");
				$db->query();
				//send email
